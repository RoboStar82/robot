...
typedef struct LidarEdge {
    uint16_t angle4 = 0;
    uint16_t length = 0;
    int16_t lengthX = 0;
    int16_t lengthY = 0;
} __attribute__((packed)) LidarEdge;
...
class AnyLidar {
   ...
   protected:
    int lineSide(LidarPoint a, LidarPoint b, LidarPoint p) {
    ...
    int lineDistance(LidarPoint a, LidarPoint b, LidarPoint p) {
        return abs((p.y - a.y) * (b.x - a.x) - (p.x - a.x) * (b.y - a.y));
    }

    void quickHull(LidarPoint points[], int pointCount, LidarPoint a, LidarPoint b, int side) {
        int maxDistanceN = -1;
        int maxDistance = -1;
        for (int n = 0; n < pointCount; n++) {
